...
- [dbt-plan](https://github.com/PresentJay/dbt-plan) - `terraform plan` for dbt: diffs compiled SQL between two revisions to predict the DDL `dbt run` would emit, and fails the pull request on a dropped column or a downstream model it breaks. Reads files only, so it runs against any warehouse without a connection.
